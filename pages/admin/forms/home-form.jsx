...
export default function MobileAppForms() {
  ...
  const fetchSubmissions = useCallback(async (page = 1) => {
    ...
    setError(null);
    try {
      let url = `/api/admin/forms/home-submissions?page=${page}&limit=10`;
                      ...
                      {submissions.length > 0 ? (
                        submissions.map((submission) => (
                          <tr key={submission.id || submission.submission_date}>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{submission.first_name}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{submission.last_name}</td>
